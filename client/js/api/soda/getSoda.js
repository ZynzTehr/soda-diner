(function () { // Immediately invoke function
    ...
    const renderSoda = ({ name, brand, fizziness, taste_rating, served }) => {
        const $title = $('#title');
        const $name = $('#name');
        const $brand = $('#brand');
    ...
    function updateSoda() {
        ...
        $.ajax({
            ...
            .done(res => {
                ...
                if (served.text() === 'false') {
                    $(this).replaceWith('<b>This soda is now being served</b>');
                    $served.text('true');
                } else {
                    $(this).replaceWith('<b> This soda is not being served any more </b>');
                    $served.text('false');
                }
            })
